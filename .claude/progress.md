...
| 5     | 운영 인프라 상향 (검색, 웹폰트, 측정) | ✅ 완료 (2026-07-19, STATUS.md 참조)     |
...
- 관찰: 08-developer-guide는 125줄로 길이 밴드 하한(200) 미달 → 후속 보강으로 해소(아래 참조)
...
- §4를 링크 표에서 4개 방법 인페이지 요약으로 확장(각 방법 핵심 설명 + 최소 예시 + 효과/한계 + 전체 예시 링크). 독자가 하위 페이지 이동 없이 이해 가능
- 상황별 적용 조합 권장 표 신설(소규모 1+3, 배포 제품 1+3+4, 팀 표준 +2)
- 반말 평서형 종결을 존댓말로 통일
- 결과: Top-5 + 08 전 챕터가 길이 밴드 충족(03=337, 04=342, 05-sbom=287, 06=220, 08=205)
