...
## 7. 1부 ②: agent 가 산출물을 만든다 · 시작 5:50 · 길이 1분
...
## 8. 1부 ③: 정확한 SBOM 을 만드는 문제 · 시작 6:50 · 길이 1분 30초
...
## 10. 1부 ⑤: 자체 인증 선언 절차 · 시작 12:50 · 길이 1분
...
## 12. 2부 ②: 4단계가 필요한 이유 · 시작 17:50 · 길이 1분 30초
...
## 13. 2부 ③: 4a — 모델에 무엇이 전달되는가 · 시작 19:20 · 길이 1분 30초
...
## 14. 2부 ④: 4b — 탐지되지 않은 영역 찾기 · 시작 20:50 · 길이 1분
...
- 4a 가 3단계 결과를 다시 보는 일이라면, 4b 는 3단계가 아예 보지 않는 영역을 대상으로 한다.
  12번에서 말씀드린 비즈니스 로직과 엣지케이스 입력 처리다
...
## 17. TRUSCA ①: 선언 이후에 남는 것 · 시작 25:20 · 길이 1분 30초

```
자체 인증을 선언한 뒤에도 관리는 계속됩니다

18974 가 요구하는 것은 지속 운영입니다 (§4.3.2)
  · 출시 후 새로 공개되는 취약점 모니터링
  · 식별된 취약점마다 조치 판단과 그 기록 유지
  · 이전에 배포한 버전에 대한 대응

선언 이후 운영을 이어가려면 도구가 필요합니다.
```
...
- 지금까지 말씀드린 키트가 해 주는 것은 체계를 세우고 산출물을 만드는 데까지다.
  선언문 초안이 나오면 거기서 끝난다
- 그런데 18974 §4.3.2 를 다시 읽어 보면 요구하는 것이 지속 운영이다. 출시 이후에도 새로
  공개되는 취약점을 모니터링해야 하고, 식별된 취약점마다 조치를 판단해 기록해야 하며,
  이미 배포한 버전에 새 취약점이 나오면 대응해야 한다.
...
- 이걸 사람이 손으로 하기는 어렵다. 도구가 필요한데 조직 여건에 따라 갖추기 어렵다.
  **선언은 했는데 운영이 이어지지 않는 상태**가 여기서 생긴다
