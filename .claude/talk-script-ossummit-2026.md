...
- **"그래서 심사를 어떻게 하느냐"는 질문이 반드시 나온다.** 세 부분으로 나눠 답한다.
  ...
  tool poisoning, tool shadowing 을 탐지한다. 명령 한 줄로 VS Code 의 MCP 설정 파일을
  검사할 수 있다
- **다만 Snyk agent-scan 은 검사 과정에서 MCP 서버를 실제로 실행한다.** 격리된 환경에서
  돌려야 하고 Snyk API 토큰이 필요하다. 이 점을 모르고 운영 환경에서 돌리면 곤란해진다
