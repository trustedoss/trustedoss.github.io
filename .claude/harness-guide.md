...
│   └── kwg-drift-checker.md [감사]  # KWG 원본 의미론적 갭 분석
│
├── skills/                           # 슬래시 커맨드 정의
...
│   ├── kwg-check/skill.md            # /kwg-check — KWG 원본 싱크 확인
...
### `validate-chain.py` — agent 체인 전제 조건 검증

```bash
# output-sample/ 기준 전체 체인 검증 (기본)
python3 .claude/scripts/validate-chain.py

# output/ 기준 검증 (실제 산출물)
python3 .claude/scripts/validate-chain.py --dir output

# 특정 agent만
python3 .claude/scripts/validate-chain.py --agent 05-sbom-management

# 상세 출력
python3 .claude/scripts/validate-chain.py -v
```

각 agent 전환 지점에서 전제 조건 파일이 실제 존재하는지 확인한다.
FAIL은 체인 단절, WARN은 조건부 파일 누락(프로필에 따라 달라지는 파일).

---

### `run-dryrun.sh` — OpenWave 드라이런 오케스트레이터

```bash
# 체인 연결 검증만 (API 불필요)
bash dry-run/run-dryrun.sh --chain-only

# 전체 E2E (ANTHROPIC_API_KEY 필요)
...
OpenWave 스타트업 프로필(SaaS·Python·기여 계획 있음·격주 배포) 기준으로
OpenWave 특화 fixture를 선택해 test-agent-e2e.py를 실행한 뒤 validate-chain.py로 체인을 검증한다.
agent CLAUDE.md 수정 후 빠른 회귀 검사에 유용하다.
...
### 시나리오 H — agent CLAUDE.md 수정 후 드라이런

```
상황: agents/ CLAUDE.md 질문 순서·분기·선택지를 수정함
```

```bash
# 1. 체인 연결만 빠르게 확인 (API 불필요, 수초 완료)
bash dry-run/run-dryrun.sh --chain-only

# 2. 수정한 agent만 E2E 검증 (API 필요, $0.10~0.30 예상)
export ANTHROPIC_API_KEY=sk-ant-...
bash dry-run/run-dryrun.sh --agent 04-process-designer
...
| 단계             | 목적                          | 실행 시점           |
| ---------------- | ----------------------------- | ------------------- |
| `--chain-only`   | 파일 누락·연결 단절 즉시 탐지 | CLAUDE.md 수정 직후 |
| `--agent <name>` | 수정된 agent 실제 동작 검증   | PR 전               |
| 전체 드라이런    | 체인 회귀 없음 확인           | 릴리즈 전           |
