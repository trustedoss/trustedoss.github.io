...
검증 항목:
  [A] requirements-matrix.md: 모든 G-항목에 담당 Agent 할당 여부
  [B] requirements-matrix.md: 모든 G-항목에 output/ 파일 할당 여부
  [C] requirements-matrix ↔ validate-checklist 일관성 (누락 파일 탐지)
...
MAPPING_FILE = ROOT / "website/reference/requirements-matrix.md"
...
def parse_mapping() -> list[dict]:
    """requirements-matrix.md에서 G-항목 데이터 파싱"""
...
def test_a(items: list[dict]):
    ...
    record(
        ...
        len(items) >= 25,
        f"requirements-matrix.md에서 {len(items)}개 파싱됨",
    )
...
def test_c(items: list[dict], validate_files: list[str]):
    print("\n[C] requirements-matrix ↔ validate-checklist 일관성 확인")
    ...
    record(
        "validate-checklist의 모든 파일이 요구사항 대조표에 커버됨",
