...
# 레포 루트 기준 절대 경로 (어느 디렉토리에서 실행해도 동작)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
TARGET_DIR="$ROOT/.claude/reference/kwg"
...
echo ""
echo "===== KWG 드리프트 감지 시작 ====="
if command -v python3 &>/dev/null && [ -f "$ROOT/.claude/scripts/check-kwg-drift.py" ]; then
  if (cd "$ROOT" && python3 .claude/scripts/check-kwg-drift.py); then
    echo "✅ KWG 싱크 이상 없음."
  else
    EXIT_CODE=$?
