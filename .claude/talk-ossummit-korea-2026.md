...
| 전체 지도                | 2분  | 두 영역 + TRUSCA 위치를 한 장에. 이후 모든 슬라이드가 이 지도 위    |
...
1부와 2부는 마지막에 하나로 이어진다. 키트가 제공하는 범위는 체계 수립과 산출물 생성까지이고,
...
- **③ 규제 요구와 대응 수단의 격차** — EU CRA 보고 의무 2026-09-11, 전면 적용 2027-12-11.
  ...
  그러나 납품되는 SBOM 은 요구 수준에 미치지 못한다. 왜 필요한지 공유되지 않은 채 제출 기한부터
  통보되고, 절차를 안내받아도 환경 구성에서 막히며, 상용 도구는 규모에 따라 부담이 크다. 무료 도구를 안내해 생성하게 해도 transitive
...
- **SBOM 정확도(1.5분)**: 문제 제기 세 번째 항목에 여기서 답한다. 누락이 생기는 원인은
...
- **문서와 운영의 차이(1.5분)**: 가이드가 제공하는 범위는 체계 설계와 산출물까지다. 정책 문서가
  있다고 금지 license 가 걸러지지 않고, 프로세스 문서가 있다고 승인 이력이 남지 않는다. 18974 는 주간 단위
  취약점 DB 갱신, VEX 판정, 라이선스 정책 게이트의 지속 운영을 요구한다. 상용 SCA 도입 부담
  때문에 선언 이후 운영이 이어지지 않는 경우가 많다
- **무엇인가(2분)**: Apache-2.0 self-hosted SCA. cdxgen 기반 30개 이상 생태계 탐지,
  Trivy 통합 DB(NVD·OSV·GHSA·EPSS·KEV, 주간 갱신), VEX 가져오기·내보내기와 7단계 triage,
