...
시간이 지연될 때 축소 순서: Q&A → TRUSCA 로드맵 1분 → 4단계 AI 방어 3분. 축 A 데모와 5단계
자가진단은 세션의 핵심이므로 마지막까지 유지한다.

### 서사의 뼈대

두 축이 마지막에 한 지점에서 합류한다. 키트가 제공하는 범위는 체계 수립과 산출물 생성까지이고,
ISO/IEC 18974 가 요구하는 것은 지속 운영이다. 인증 선언으로 종료되지 않는다는 점을 청중이
인식한 시점에 TRUSCA 를 제시하면 홍보가 아니라 논리적 귀결로 전달된다. TRUSCA 로드맵의
1순위가 "에이전트가 패키지를 반입하기 전에 정책을 조회하는 MCP 서버"이므로, 축 B 의 에이전트
거버넌스와 직접 연결된다.
...
- **② AI 코딩이 바꾼 조건** — 세션 제목의 절반을 여기서 설명한다. 세 가지다.
  ...
  기존 통제는 사람이 코드를 작성한다는 전제 위에 설계되었다는 점이 요지다 → **축 B 로 연결**
- **③ 규제 요구와 대응 수단의 격차** — EU CRA 보고 의무 2026-09-11, 전면 적용 2027-12-11.
  CISA 2026 최소 요소(2026-07-29, 행사 2주 전)가 NTIA 2021 을 대체하며 컴포넌트 해시와
  라이선스가 필수 항목으로 변경되었고 적용 범위에 AI 소프트웨어와 SaaS 가 포함되었다.
  그러나 다수의 공급사는 SBOM 을 생성하지 못한다 — 소스 없음, 폐쇄망, 충족 여부 판정 불가
  → **8번 슬라이드(BomLens)로 연결**

CISA 기준 변경을 **이 가이드가** 이미 반영했다는 점을 한 줄 언급한다. 도구(BomLens·TRUSCA)의
구현 여부는 별개 사안이므로 혼동하지 않는다.

③이 축 A 의 BomLens 대목과 문제·해결 쌍을 이룬다. 이 구조 덕분에 BomLens 는 도구 소개가 아니라
