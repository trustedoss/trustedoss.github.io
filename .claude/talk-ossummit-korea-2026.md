...
- **③ 규제 요구와 대응 수단의 격차** — EU CRA 보고 의무 2026-09-11, 전면 적용 2027-12-11.
  ...
  그러나 다수의 공급사는 요구 수준의 SBOM 을 생성하지 못한다 — 소스 없음(binary·firmware),
  최소 요소 충족 여부 판정 불가. 폐쇄망은 여기에 넣지 않는다(로컬 실행 도구로 생성은 가능하며,
  제약이 되는 것은 SaaS 형 SCA 이므로 TRUSCA 구간의 논점이다)
...
별도 층으로 길게 설명하지 않는다 — 5단계 모델의 3단계가 곧 DevSecOps이므로 2부에서 흡수한다.
