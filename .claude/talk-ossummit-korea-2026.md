...
별도 구간을 만들지 않는다. 발표자가 SK텔레콤 소속인데 SK텔레콤 도구와 자기 조직 도구를 연달아
소개하면 후반 10분이 통째로 도구 소개가 되어 "No product pitches" 인상이 생긴다. 두 곳에 한 줄씩
넣으면 추가 시간 없이 소화된다.

- **축 A, SBOM 생성 대목** — syft·cdxgen과 나란히 선택지로. 소스·컨테이너·바이너리·펌웨어·ML-BOM을
  한 도구로 다루고 로컬에서 돈다는 점이 폐쇄망 청중에게 실질적인 정보다. Apache-2.0 명시
- **TRUSCA 구간** — TRUSCA의 규제 크로스워크 데이터가 BomLens 에서 가져온 것이라는 사실
  (TRUSCA CHANGELOG Unreleased 항목, `THIRD_PARTY_NOTICES.md` 에 귀속 표기). 제품 소개가 아니라
  **한국 오픈소스 생태계가 실제로 데이터를 주고받는 사례**로 말한다. OpenChain 한국 워킹그룹
  리드라는 위치에서 가장 자연스러운 각도이고, 홍보 인상도 없다
