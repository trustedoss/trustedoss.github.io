...
잡고, 남는 5분은 입퇴장과 장비 전환 여유로 둔다. 리허설도 35분 기준으로 실측한다.

| 구간                      | 시간 | 내용                                                                |
| ------------------------- | ---- | ------------------------------------------------------------------- |
| 문제 제기                 | 3분  | 사고 사례 → 규제(CISA 2026·EU CRA) → AI 코딩이 더한 새 위험         |
| 전체 지도                 | 2분  | 키트 3층 + TRUSCA 위치를 한 장에. 이후 모든 슬라이드가 이 지도 위   |
| 축 A — ISO 자체 인증 키트 | 9분  | 흐름 2분 / SBOM 생성의 간극 1.5분 / 녹화 데모 4.5분 / 등재 절차 1분 |
...
시간이 밀리면 줄이는 순서: Q&A → TRUSCA 로드맵 1분 → 4단계 AI 방어 3분. 축 A 데모와 5단계
자가진단은 세션의 핵심이므로 마지막까지 지킨다.
...
- **규제는 계속 올라간다.** EU CRA 보고 의무 2026-09-11 시작, 전면 적용 2027-12-11.
  **CISA 2026 SBOM 최소 요소**(2026-07-29, 행사 2주 전)가 NTIA 2021을 대체하며 컴포넌트 해시와
  라이선스를 필수 필드로 올렸고, AI 소프트웨어와 SaaS까지 범위를 넓혔다
- **그런데 공급사 다수는 정확한 SBOM을 만들지 못한다.** 소스 없이 바이너리나 펌웨어만 납품하는
  경우, 폐쇄망이라 외부 서비스에 올릴 수 없는 경우, 최소 요소 충족 여부를 판정할 방법이 없는
  경우. 요구 수준은 올라가는데 만들 수단이 따라가지 못하는 간극이 이 발표의 문제 정의다
- **AI 코딩이 위험을 증폭시킨다.** 사람이 검토하지 않은 의존성이 공급망에 들어온다

CISA 기준 변경을 **이 가이드가** 이미 반영해 두었다는 점을 "살아 있는 프로젝트"의 증거로
한 줄 언급한다. 도구(BomLens·TRUSCA)의 구현 여부는 별개 사안이므로 섞지 않는다.

두 번째 항목이 축 A의 BomLens 대목과 문제·해결 쌍을 이룬다. 이 구조 덕분에 BomLens 는 도구
소개가 아니라 발표가 제기한 문제의 답 위치에 놓인다.
...
- 흐름(2분): Agent 9종 → 산출물 24종 → 갭 분석 → 선언문. 5230과 18974의 공통 기반이 크다는 점.
  산출물을 일일이 나열하지 말고 화면으로 넘긴다
- **SBOM 생성의 간극(1.5분)**: 문제 제기 두 번째 항목을 여기서 받는다. 소스가 있는 프로젝트는
  syft·cdxgen 으로 되지만, 소스 없는 납품물과 폐쇄망은 답이 없다. **BomLens**(Apache-2.0)가
  이 공백을 메운다 — 로컬에서 실행되고 소스·컨테이너·바이너리·펌웨어를 입력으로 받으며,
  CycloneDX SBOM 과 함께 고지문·위험 리포트·보안 리포트를 한 번에 낸다. ML-BOM 도 생성한다.
  "최적"이라고 단정하지 말고 **"펌웨어와 바이너리에서도 만들 수 있다"**는 구체적 능력으로 말한다
...
축도 다르다. 5단계는 AI가 만든 코드의 위험을 다루고, TRUSCA는 의존성 위험을 다룬다. 사람이
...
SBOM 을 만들어야 하는데 쓸 만한 수단이 없어 못 만든다. 이 사실을 문제 제기에서 세우고 축 A 에서
받는 구조로 배치한다. 그러면 도구 소개가 아니라 발표가 제기한 문제의 답이 된다.

- **축 A, SBOM 생성 대목 (1.5분, 주 배치)** — 소스 없는 납품물과 폐쇄망이라는 공백을 명시한 뒤
...
| 5.4 AI SBOM 챕터(BomLens 실측)    | 축 A SBOM 생성의 간극 1.5분 (주 배치)        |
