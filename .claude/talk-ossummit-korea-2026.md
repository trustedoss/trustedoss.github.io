...
| 1부 — ISO 자체 인증 키트 | 9분   | 6~10     | 두 표준 1 / agent 1 / 데모 4.5 / SBOM 정확도 1.5 / 등재 1                     |
...
8번 데모, 11번 자가진단, 9번 SBOM 정확도는 세션의 핵심이므로 마지막까지 유지한다.
...
- [x] 슬라이드 구성안 + 발표 스크립트 작성 — `.claude/talk-script-ossummit-2026.md`
      (21장, 발표 30분 50초 + Q&A 4분 30초). 슬라이드는 한국어 문장 + 영어 키워드
...
- [ ] `postmark-mcp` 사례 슬라이드 — 사실 경계 주의(악성 시작 버전은 추정, ActiveCampaign
      연관은 미확인)
- [ ] SBOM 정확도 슬라이드 + BomLens 1장 — 왼쪽에 누락이 생기는 원인(빌드 환경 불일치),
      오른쪽에 BomLens 의 입력과 출력을 두면 1.5분에 들어간다
- [ ] 4b AI fuzzing 슬라이드 1장 — 대상·방법·탐지 대상과 도구 조합 3행
- [ ] agent tool 통제 슬라이드에 심사 도구 행 추가
- [ ] TRUSCA 3장 (문서와 운영의 차이 / 무엇인가 / 로드맵)
- [ ] 마무리 슬라이드 — 세 도구를 쓰는 시점별로 구분
...
| 5.4 AI SBOM 챕터(BomLens 실측)    | 1부 SBOM 정확도 1.5분 (주 배치)             |
