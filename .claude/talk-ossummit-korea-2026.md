...
- **SBOM 정확도(1.5분)**: 문제 제기 세 번째 항목을 여기서 받는다. 무료 도구로 생성하는 것
  자체는 어렵지 않고, 문제는 결과물의 정확도다 — transitive dependency 누락, PURL 누락,
  license 미확인. 최소 요소를 못 채워 납품처가 반려하면 공급사는 원인을 모른 채 다시 만든다.
  **BomLens**(Apache-2.0)를 답으로 제시하되, 확인된 사실(입력 범위, 출력물, ML-BOM 의 G7 AI
  최소 요소 점검)로만 말한다. 일반 SBOM 의 정확도 우위는 공개 문서에서 확인되지 않았다
