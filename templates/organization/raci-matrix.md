# 오픈소스 RACI 매트릭스

<!-- 5230 §3.2.2.1·§3.2.2.2·§3.2.2.4, 18974 §4.2.2.1·§4.2.2.2·§4.2.2.4 -->

**회사명**: {회사명}
**작성일**: YYYY-MM-DD

> R=Responsible(실행), A=Accountable(승인), C=Consulted(자문), I=Informed(보고)

---

## RACI 매트릭스

| 업무                                  | 오픈소스 담당자 | 개발팀 | 보안팀 | 법무 | 경영진 |
| ------------------------------------- | :-------------: | :----: | :----: | :--: | :----: |
| 오픈소스 사용 검토 및 승인            |        A        |   R    |   C    |  C   |   I    |
| 라이선스 컴플라이언스 검토            |        R        |   C    |   I    |  C   |   I    |
| SBOM 생성 및 관리                     |        A        |   R    |   I    |  I   |   I    |
| 취약점 스캔 및 대응                   |        C        |   R    |   R    |  I   |   I    |
| 정책 수립 및 갱신                     |        R        |   C    |   C    |  C   |   A    |
| 교육 프로그램 운영                    |        R        |   I    |   I    |  I   |   I    |
| 외부 라이선스 문의 대응               |        R        |   C    |   I    |  C   |   I    |
| 외부 보안 취약점 신고 대응            |        C        |   I    |   R    |  I   |   I    |
| 자체 인증 선언                        |        R        |   I    |   C    |  C   |   A    |
| **오픈소스 기여 활동 승인**           |        A        |   R    |   I    |  C   |   I    |
| **사내 프로젝트 공개 검토**           |        A        |   R    |   C    |  A   |   I    |
| **외부 라이선스·보안 문의 종합 대응** |        R        |   I    |   C    |  C   |   I    |

---

## 역할별 담당자

<!-- 5230 §3.2.2.1·§3.2.2.2 -->

| 역할            | 담당자 이름 | 부서        | 이메일   | 전담/겸직     |
| --------------- | ----------- | ----------- | -------- | ------------- |
| 오픈소스 담당자 | {이름}      | {부서}      | {이메일} | {전담/겸직}   |
| 개발팀 대표     | {이름}      | {부서}      | {이메일} | 겸직          |
| 보안 담당       | {이름/부서} | {부서}      | {이메일} | {전담/겸직}   |
| 법무 담당       | {이름/외부} | {부서/외부} | {이메일} | {상시/필요시} |

---

## 예산 배분 현황

<!-- 5230 §3.2.2.2, 18974 §4.2.2.2 -->

| 항목               | 현황                        |
| ------------------ | --------------------------- |
| 전담 인력 배치     | {전담 N명 / 겸직 N명}       |
| 오픈소스 도구 예산 | {있음 / 없음 / 연 {금액}}   |
| 법률 자문 예산     | {있음 / 없음 / 필요시 청구} |
| 외부 교육 예산     | {있음 / 없음 / 연 {금액}}   |

---

## 미준수 사례 검토 절차

<!-- 5230 §3.2.2.5 -->

라이선스 미준수 사례 발생 시:

1. 담당자가 미준수 내용 파악 및 기록
2. 법무 자문을 통해 위반 수준 평가
3. 시정 조치 계획 수립 (라이선스 교체, 소스코드 공개 등)
4. `output/process/usage-approval.md` 프로세스에 따라 재검토
5. 재발 방지를 위한 정책/프로세스 보완

---

## 내부 책임 할당 절차

<!-- 5230 §3.2.2.4, 18974 §4.2.2.4 -->

신규 오픈소스 관련 업무 발생 시:

1. 오픈소스 담당자가 업무 내용 정의
2. RACI 매트릭스 기준으로 담당자 배정
3. role-definition.md 및 이 문서 업데이트
