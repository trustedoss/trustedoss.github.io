...
파일: CLAUDE.md
형식: ## 섹션 헤더 기반 지시문
특이사항: 기존 파일이 PROJECT_PATH에 있으면 ## 오픈소스 정책 섹션만 추가 (기존 내용 보존)

Cursor:
파일: .cursor/rules/oss-policy.mdc
형식: frontmatter(description, alwaysApply: true) + 간결한 규칙 목록
특이사항: 레거시 .cursorrules 단일 파일은 생성하지 않는다
...
Aider:
파일: CONVENTIONS.md
형식: 프로젝트 지침 목록 (.aider.conf.yml 에 read: CONVENTIONS.md 등록 안내 포함)
