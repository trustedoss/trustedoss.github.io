...
모든 하위 agent에 공통 적용되는 규칙이다.

객관식 선택지를 제시할 때는 반드시 **번호 목록**을 사용한다. `-` 불릿은 금지한다.
...
| output/sbom/ 있음, output/sbom/sbom-management-plan.md 없음          | → agents/05-sbom-analyst → agents/05-sbom-management 순서로 실행 |
| output/sbom/sbom-management-plan.md 있음, output/vulnerability/ 없음 | → agents/05-vulnerability-analyst                                |
| output/vulnerability/ 있음, output/training/ 없음                    | → agents/06-training-manager                                     |
