...
이 템플릿은 AI 코딩 도구가 코드를 생성할 때 자동으로 오픈소스 정책을 인지하도록 만드는 공통 규칙 모음입니다. CLAUDE.md, .cursor/rules, .clinerules 같은 각 도구의 설정 파일에 붙여넣어 사용합니다. 2026년 현재 Cursor, GitHub Copilot, Windsurf, Cline 이 모두 공통 규칙 파일 AGENTS.md 를 공식 지원하므로, AGENTS.md 한 벌을 기본으로 두고 도구별 파일은 차이점만 보완하는 구성도 가능합니다. 허용·금지 라이선스 목록은 사내 정책에 맞게 수정해서 사용하시기 바랍니다.
