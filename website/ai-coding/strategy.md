...
4a 와 4b 가 이 사각지대를 AI 로 보완합니다. 4c 는 방향이 반대입니다. AI 가 무엇을 불러오는지를
...
감사 로그, 데이터 반출 경로 판정 여섯 가지이며 도입 전 스캔이 이를 뒷받침합니다.

- [에이전트와 MCP 도구 거버넌스](./agent-governance) — 여섯 가지 통제와 스캐너, 복붙용 조직 정책
