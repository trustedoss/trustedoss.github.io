---
id: cicd-quick
title: 30분 Quick CI/CD
sidebar_label: 30분 Quick CI/CD
sidebar_position: 4
---
...
`CLAUDE.md`나 `.cursor/rules` 같은 AI 규칙 파일은 어디까지나 "권장사항"입니다. 개발자가 의도적으로 무시하거나 AI가 실수로 규칙을 따르지 않아도 아무것도 막아주지 않습니다.

규칙 파일만 운영하는 환경에서는 PR이 병합되는 순간 정책 위반 코드가 프로덕션으로 그대로 흘러들어갈 수 있습니다. AI 도구가 빠르게 코드를 생성할수록 이 위험은 커집니다.

CI/CD 게이트는 사람과 AI의 실수에 관계없이 기계적으로 정책 위반을 차단하는 유일한 안전망입니다. 5단계 전략의 3단계([보장 수준별 5단계 전략](./strategy) 참고)부터 진정한 Hard Block이 작동합니다.
...
세 도구 모두 무료 오픈소스입니다. GitHub Actions에서는 마켓플레이스 Action으로 설치 없이 바로 쓰고, GitLab CI에서는 스크립트 한 줄로 설치해 사용합니다.
...
oss-check:
  ...
  image: ubuntu:22.04
  script:
    # 도구 설치 (ubuntu 기본 이미지에는 curl이 없음)
...
`severity-cutoff`(GitHub Actions) 또는 `--fail-on`(GitLab CI) 값을 예시의 `high`에서 `critical`로 완화하거나 `medium`으로 강화할 수 있습니다. 옵션을 생략하면 scan-action 기본값인 `medium`이 적용되므로, 의도한 기준을 명시해 두는 편이 안전합니다. 처음 파이프라인을 도입할 때는 `critical`부터 시작해 빌드 실패 빈도를 파악한 뒤 점진적으로 강화하는 방식을 권장합니다.
