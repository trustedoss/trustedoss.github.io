...
Open source licenses split into two broad camps based on one question: "if I use this code, do I have to release my own code too?"

- **Permissive**: keep the copyright notice and you're free to use it — no obligation to release your own code. Examples: MIT, Apache-2.0.
- **Copyleft**: using this code obligates you to release your own code under the same license. The scope of that obligation varies:
  - **Weak Copyleft**: only if you _modify_ the library itself do you have to release those changes (using it unmodified carries no obligation). Example: LGPL.
  - **Strong Copyleft**: the moment you _distribute_ a product containing this code, you must release the product's entire source. Example: GPL.
  - **Network Copyleft**: even if you never distribute it — just running it as a service (SaaS) — the source-release obligation still kicks in. Example: AGPL, which is why SaaS companies need to watch it more closely than GPL.
...
This check just confirms Claude Code is installed and running correctly. The session running now
is separate from the dedicated 03-policy-generator session you open in Step 1 below. When you start
the exercise, you'll end this session and open a new one.
:::
...
To start over, run `claude` again in the same folder (`agents/en/03-policy-generator`) and answer
the questions again. The existing files are overwritten with the new content.
:::
