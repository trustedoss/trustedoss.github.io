...
SBOM을 한 번 생성하는 것만으로는 부족하다. 소프트웨어는 지속적으로 변경되고, 새로운 취약점이 매일 공개됩니다. 이 챕터에서는 생성한 SBOM을 릴리즈 주기에 맞춰 갱신하고, 버전별로 보관하며, 외부 고객이나 납품처에 체계적으로 공유하는 프로세스를 수립합니다.
...
소프트웨어가 변경될 때마다 SBOM도 변경되어야 합니다. 관리되지 않는 오래된 SBOM은 없는 것보다 위험할 수 있습니다. 최신 상태가 아닌 SBOM을 신뢰하면 실제 구성 요소와 문서 사이의 불일치로 인해 보안 사각지대가 생긴다.

**실제 사례:** 6개월 전 생성한 SBOM을 그대로 신뢰했다가, 그사이 추가된 취약한 라이브러리를 발견하지 못한 경우가 있습니다. 해당 라이브러리에서 CVE가 공개된 후에도 조직은 자신이 영향받는다는 사실을 인지하지 못했고, 고객사에서 먼저 문제를 발견하여 신뢰를 잃었다.
...
SBOM 관리 체계가 갖춰졌으면, 취약점 분석 단계로 이동합니다. SBOM을 기반으로 현재 소프트웨어에 영향받는 CVE를 분석하고, 대응 계획을 수립하는 단계다.
