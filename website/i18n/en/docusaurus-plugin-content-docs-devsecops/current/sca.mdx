...
The analyzer above is for one-off checks. If your whole team needs a continuously running,
self-hosted SCA, move on to [TRUSCA](https://trustedoss.github.io/trusca/) — an Apache-2.0 tool
that manages vulnerabilities (CVEs), license compliance, and SBOMs in one UI, no commercial SCA
required.

- Try it yourself: [TRUSCA repository](https://github.com/trustedoss/trusca) (Docker Compose or Helm)
- It is a free path to a continuously operated SCA where adopting a commercial tool is hard (budget, staffing, air-gapped networks).
  :::
