...
This template is a common ruleset that makes AI coding tools automatically recognize open source policy when generating code. Paste it into each tool's configuration file such as `CLAUDE.md`, `.cursor/rules`, or `.clinerules`. As of 2026, Cursor, GitHub Copilot, Windsurf, and Cline all officially support the common rules file `AGENTS.md`, so you can also keep a single AGENTS.md as the baseline and let per-tool files carry only the differences. Please modify the allowed/forbidden license lists to match your internal policy.
