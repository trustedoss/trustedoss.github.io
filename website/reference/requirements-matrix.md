...
이 페이지는 ISO/IEC 5230과 ISO/IEC 18974의 **31개 요구사항을 항목 단위로 펼친 정본**입니다. 항목마다 OpenChain 자체 인증 체크리스트의 질문 원문, 표준 원문의 입증자료, 그것을 채우는 산출물 파일, 그 파일을 만드는 에이전트를 함께 싣습니다.

처음 읽는 단계에서는 이 페이지가 필요하지 않습니다. 두 표준이 무엇을 요구하는지 먼저 파악하려면 [표준 요구사항 한눈에](/docs/overview/checklist-mapping) 요약을 보세요. 이 대조표는 [자체 인증 선언](/docs/conformance) 단계에서 갭 분석 결과를 항목별로 확인할 때 씁니다.

:::info 항목 ID 체계
`G1` 프로그램 기반, `G2` 관련 업무 정의, `G3-L` 라이선스 컴플라이언스, `G3-S` 보안 보증, `G3-B` SBOM과 공급망, `G4` 준수 선언과 유지. 그룹별 설명은 [표준 요구사항 한눈에](/docs/overview/checklist-mapping)에 있습니다.
:::
...
:::info[§4.1.2.5 · §4.1.2.6 초기 인증 시 처리]
...
식별된 취약점에 대해 조치가 완료됐거나 조치 불요 판정이 기록됐음을 배포 전 확인. 18974는 알려진
취약점이 하나도 없을 것을 요구하지 않고, 탐지, 평가, 조치, 기록 절차가 돌아가고 그 결과가 문서로
남아 있을 것을 요구합니다. 잔여 CVE가 있어도 조치 판단과 기록이 있으면 선언할 수 있습니다.
...
- [표준 요구사항 한눈에](/docs/overview/checklist-mapping) — 두 표준이 무엇을 요구하는지 요약
- [자체 인증 선언](/docs/conformance) — 이 대조표로 갭 분석 결과를 확인하는 단계
- [AI 에이전트로 산출물 만들기](./agents) — 담당 에이전트별 실행 방법
