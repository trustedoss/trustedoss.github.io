...
:::info[참고]
위 기한은 ISO/IEC 18974 및 OpenChain KWG 기준선입니다. 조직 역량이 허용하는 경우 Critical 24시간·High 1주일 같은 더 엄격한 기한을 내부 표준으로 적용할 수 있습니다.
:::
...
§9가 _언제_ 검토하는지를 정한다면, 이 절은 운영 중 누구나 변경을 요청하고 처리하는 절차를 정한다.
...
- **담당**: DevOps팀 오픈소스 담당자가 분기 1회 이상 관련 규제와 표준 동향(EU CRA, 국내 SBOM 의무화, ISO 개정 등)을 점검한다.
...
```
관련 표준
- KWG 정책 템플릿 §8 (사내 프로젝트 오픈소스 공개)
```

테크유니콘 내부에서 개발한 소프트웨어를 오픈소스로 공개할 때는 사전 승인, 지식재산과 보안 검토, 라이선스 선택, 고지문 준비, 공개 후 관리를 거친다. 상세 절차는 `output/process/project-publication-process.md`(프로세스 산출물)를 따른다.
