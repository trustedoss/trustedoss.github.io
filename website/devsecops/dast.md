...
| 보내는 요청      | 일반적인 조회 요청                                      | SQL 삽입, 경로 순회, 명령 삽입 등 실제 공격 문자열          |
| 찾는 것          | 응답에 드러나는 문제(헤더 누락, 쿠키 플래그, 정보 노출) | 실제로 취약점이 성립하는지                                  |
| 대상에 남는 흔적 | 조회 로그                                               | 공격 시도 로그, 새로 생기거나 바뀐 데이터, 실패한 인증 시도 |

Baseline은 공격 페이로드를 보내지 않습니다. 그래서 사람이 쓰고 있는 환경에 걸어도 무리가 없습니다. 대신 응답에 드러나는 것만 봅니다.

### 운영 중인 대상에 Full Scan을 걸 때

Full Scan은 성격이 다릅니다. 아래 셋은 대상이 읽기 전용이더라도 남습니다.
...
- **데이터 오염.** 쓰기가 막혀 있어도 예외적으로 열린 경로가 있으면 그쪽으로 들어갑니다. 스캔 결과 업로드나 웹훅 수신처럼 외부 입력을 받도록 설계된 경로가 대표적입니다.
...
데모 호스트가 `DEMO_READ_ONLY` 로 GET과 HEAD, OPTIONS 외의 요청을 거부하고 있어 쓰기 위험은 대부분 막혀 있었습니다. 그런데도 Full을 미룬 이유는 위의 세 가지 중 두 가지가 읽기 전용과 무관하게 남기 때문입니다. 조회만 해도 자원은 소모되고, 인증 시도는 잠금 정책을 건드립니다. 게다가 샌드박스 스캔을 허용하는 설정이 켜지면 인제스트 경로가 열립니다. Full은 스캔 전용 인스턴스가 생긴 뒤로 미뤘습니다.

Baseline에도 상한을 걸었습니다. 스파이더 5분, 전체 실행 10분입니다. 실행 시간을 줄이려는 것이 아니라, 제한 없는 스파이더가 인제스트 경로를 반복해서 두드리는 것 자체가 공격 페이로드 없이도 자원 고갈이 되기 때문입니다.
