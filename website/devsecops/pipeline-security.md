...
tj-actions 사건과 공격 기법은 다르지만(태그 재지정이 아니라 입력값 처리 결함), 결론은 같습니다.
보안 스캔을 위해 넣은 워크플로 단계 자체가 검증 대상에서 빠지면 그 자체가 침투 경로가 됩니다.

두 사건이 보여주는 공통 교훈은, 워크플로가 실행하는 참조와 입력값 모두 검증 대상이라는
것입니다. tj-actions는 내용이 고정되지 않은 태그 참조가, Trivy Action은 검증 없이 신뢰한
입력값이 문제였습니다.
