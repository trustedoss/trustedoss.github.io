...
**Trivy Action 태그 재지정 (2026-03).** 같은 방식의 공격이 보안 스캐너를 대상으로 반복됐습니다.
컨테이너 이미지를 검사하려고 넣은 스캔 단계가 그 자체로 침투 경로가 된 사례입니다.
보안 도구라는 이유로 검증에서 빼 두면 안 된다는 것을 보여줍니다.

두 사건 모두 근본 원인은 같습니다. 워크플로가 내용이 고정되지 않은 참조를 실행했다는 것입니다.
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: aquasecurity/trivy-action@<40자리 커밋 SHA> # 0.36.0
```
