...
CodeQL은 GitHub 전용 도구로(GitLab CI에서는 위 Semgrep의 GitLab 예시를 사용), GitHub Actions에서 퍼블릭 저장소 및 GitHub Code Security(구 GitHub Advanced Security 의 분리 제품) 구독 조직에 무료로 제공됩니다. 데이터플로우 분석을 지원해 간접적인 취약점 경로도 탐지할 수 있습니다.
...
TRUSCA 는 SAST 를 두 워크플로우로 나눠 운영합니다.
[sast.yml](https://github.com/trustedoss/trusca/blob/main/.github/workflows/sast.yml) 은 bandit(High 심각도)과 semgrep(ERROR 심각도)을 각각 Hard Fail 기준으로
돌리고, [codeql.yml](https://github.com/trustedoss/trusca/blob/main/.github/workflows/codeql.yml) 은 CodeQL 분석을 별도로 수행합니다.

같은 SAST 라도 도구마다 잡는 것이 달라 겹쳐 쓰는 구성이며, 차단 기준을 심각도로 명확히 끊어
오탐 때문에 개발이 멈추지 않도록 한 점을 참고할 만합니다.
