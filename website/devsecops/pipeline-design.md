...
아래 통합 워크플로의 각 잡은 영역별 페이지의 설정을 그대로 가져온 것입니다. SCA 잡의 `.grype.yaml` 예외 관리와 보관 정책은 [SCA](./sca)에, 나머지 영역은 각 페이지에 설명이 있습니다.
...
오픈소스 프로젝트 TRUSCA 는 워크플로우를 스물다섯 개로 나눠 운영합니다. 트리거를 기준으로 보면
설계 의도가 드러납니다.
...
개발자가 우회할 방법을 찾기 때문입니다.

같은 검사를 두 시점에 두는 경우도 있습니다. codeql 은 PR 에서 변경분을 보고, 매주 월요일
01:30 에 전체를 다시 봅니다. 새 규칙이 나중에 추가되어도 기존 코드가 재검사됩니다.
