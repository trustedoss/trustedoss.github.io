...
ISO/IEC 5230(라이선스 컴플라이언스)과 ISO/IEC 18974(보안 보증)는 각각 정책 문서에 포함되어야 할 항목을 정의합니다.
...
| 알려진 취약점 대응 절차     | —            | ✓             |
| 보안 취약점 공개 정책       | —            | ✓             |
| **프로그램 성과 지표(KPI)** | —            | ✓ (§4.1.4.2)  |

두 표준 모두 정책 문서의 존재만으로는 부족하며, 실제로 직원들이 접근할 수 있고 인지하고 있어야 한다고 요구합니다.
...
| KPI 예시                              | 측정 방법                                     |
| ------------------------------------- | --------------------------------------------- |
| CVE 탐지 후 Critical 취약점 패치 기간 | 발견일 ~ 패치 배포일 (목표: 7일 이내)         |
| SBOM 갱신 주기 준수율                 | 릴리즈 시 SBOM 업데이트 횟수 / 전체 릴리즈 수 |
| 오픈소스 사용 승인률                  | 승인 요청 대비 처리 기간 (목표: 3영업일 이내) |
