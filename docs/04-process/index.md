...
강사가 정책과 프로세스의 차이를 설명하고, 이 챕터의 Mermaid 흐름도(오픈소스 라이프사이클)를
함께 리뷰한다. 각 팀이 현재 어떤 방식으로 오픈소스를 도입하고 있는지 간단히 공유한다.

**2단계 (40분): agent 실행 및 4개 문서 생성**
...
4개 질문에 답변하며 `output/process/` 디렉토리에 4개 파일을 생성한다.
시간이 부족하면 질문 4개 답변과 파일 생성을 최우선으로 완료한다.

**3단계 (10분): process-diagram.md Mermaid 흐름도 팀 리뷰**

각 팀이 생성된 `process-diagram.md`를 GitHub에서 열어 흐름도를 확인한다.
팀별로 다르게 나온 부분(결재 단계, CI/CD 통합 방식 등)을 공유하고 비교한다.

### 막혔을 때

- **CI/CD가 없으면**: "없음"으로 답변. agent가 수동 체크리스트 기반 프로세스를 생성한다.
- **결재 단계가 모호하면**: 현재 팀에서 사용하는 방식을 그대로 입력. 나중에 수정 가능.
- **Mermaid가 렌더링되지 않으면**: [mermaid.live](https://mermaid.live) 에서 직접 확인.
