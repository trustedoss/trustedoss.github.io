...
기여 계획이 없는 경우 agent는 이 문서를 생성하지 않으며, 갭 분석에서 해당 항목은 N/A로 처리됩니다. 원한다면 "현재 기여 계획 없음 — 향후 계획 수립 시 이 절차를 따른다"는 선언적 문서를 직접 작성해 둘 수도 있습니다.
...
공개 계획이 없는 경우 agent는 이 문서를 생성하지 않으며, 갭 분석에서 해당 항목은 N/A로 처리됩니다. 원한다면 "현재 공개 계획 없음 — 공개 결정 시 이 절차를 따른다"는 선언적 문서를 직접 작성해 둘 수도 있습니다.
...
생성되는 파일은 아래 [예상 결과물](#예상-결과물) 표와 같습니다.
