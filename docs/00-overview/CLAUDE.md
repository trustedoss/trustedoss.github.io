...
2. **요구사항 네 덩어리** → G1~G4 그룹별로 어느 챕터가 채우는지 확인
3. **요약 통계** → 전체 31개 항목과 39% 절감 근거 확인

항목별 담당 Agent, 산출물 파일, 인증 질문, 비고 태그(`[공통]` `[5230]` `[18974]` `[공급망]` `[규제]`)는 `website/reference/requirements-matrix.md` 에 있다. `verify.sh` [8/13] 의 `test-coverage.py` 가 파싱하는 파일도 이쪽이다.
