...
> ⚠️ **§4.1.2.5 · §4.1.2.6 초기 인증 시 처리**: 처음 인증할 때는 검토 이력이 없으므로 부분충족(🔶)으로 처리한다.
> gap-analysis.md에 "검토 주기 계획"과 "담당자 지정"을 기록하고, 18개월 후 갱신 시 실제 이력으로 충족한다.
...
> ⚠️ **§4.1.4.3 초기 인증 시 처리**: 처음 인증할 때는 개선 이력이 없다. 초기 갭 분석 실행 자체를 1회 감사 이력으로 gap-analysis.md에 기록하고, 18개월 후 갱신 시 2회 이상 이력으로 충족한다.
