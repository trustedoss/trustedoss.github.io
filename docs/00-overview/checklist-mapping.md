...
1. **두 표준 비교 표** → 각 표준의 목적과 범위를 먼저 파악합니다
2. **통합 매핑** → G1~G4 그룹별 항목 블록에서 입증자료·산출물·담당 Agent를 확인합니다
3. **태그** → `[공통]` `[5230]` `[18974]` `[공급망]` `[규제]` 로 항목 성격을 빠르게 파악합니다
4. **요약 통계** → 문서 하단에서 전체 현황을 숫자로 확인합니다
...
:::info[참고]
공통 항목(12개)은 5230(20개)과 18974(23개) 양쪽에 모두 계산됩니다. 두 표준을 동시에 준비하면 공통 항목을 한 번만 작업하여 약 39%(12/31)를 절약할 수 있습니다.
:::
...
:::info 셀프스터디 모드 (약 1시간)
이 매핑 문서를 파악했으면 실제 산출물 생성을 시작하세요.
...
6. **SBOM 관리 계획** → `cd agents/05-sbom-management && claude`
7. **취약점 분석** → `cd agents/05-vulnerability-analyst && claude`
