...
이 챕터에서는 SBOM의 개념, 최소 필수 요소, 대표 포맷, 그리고 SBOM 생태계를 이해한다.

실습은 없다. 읽기와 이해에 집중하면 된다.

이후 `05-tools/sbom-generation` 챕터에서 실제 SBOM을 생성할 때 이 배경 지식이 기반이 된다.
...
SBOM(Software Bill of Materials)은 소프트웨어에 포함된 **모든 구성요소의 목록**이다.
오픈소스 라이브러리, 프레임워크, 런타임, 빌드 도구 등 소프트웨어를 구성하는 모든 재료를 나열한다.

### 식품 성분표 비유

식품 포장지에는 "밀가루, 설탕, 계란, 버터 ..."가 적혀 있다.
SBOM은 소프트웨어의 성분표다.

> "이 소프트웨어에는 React 18.2.0, axios 1.4.0, log4j 2.14.0이 들어있습니다."

소비자(납품처, 고객, 규제기관)는 이 목록을 보고 안전성과 라이선스를 확인한다.
...
PURL(Package URL)은 패키지를 전 세계적으로 유일하게 식별하는 표준 형식이다.
...
PURL이 있으면 취약점 데이터베이스(NVD, OSV)와 자동으로 매핑하여 CVE를 찾을 수 있다.

---

## 4. SBOM 포맷 비교

현재 업계에서 주로 사용하는 두 가지 표준 포맷이 있다.
...
두 도구 모두 `05-tools/sbom-generation` 챕터에서 실습한다.
...
이 문서를 읽었다면 SBOM의 개념과 생태계를 충분히 이해한 것이다.

다음으로 `docs/01-setup/` 으로 이동하여 실습 환경을 준비한다.
syft, cdxgen, Dependency-Track 설치를 완료하면 본격적인 실습을 시작할 수 있다.
