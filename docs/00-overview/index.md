...
| 경로 | 대상 | 소요 시간 | 진행 방식 |
|------|------|----------|----------|
| 셀프스터디 | 혼자 진행하는 담당자 | 3~5일, 총 8~12시간 | `docs/` 챕터를 00부터 순서대로 진행 |
...
3. `docs/00b-supply-chain/` 챕터로 이동 — 소프트웨어 공급망 보안 배경 지식 습득
...
- [ ] 다음 단계(챕터 `00b` 또는 챕터 `01`)로 이동할 준비가 됐다
...
**배경 지식이 필요하다면:**
→ `docs/00b-supply-chain/` 으로 이동하여 소프트웨어 공급망 보안과 SBOM 개념을 먼저 학습한다.
