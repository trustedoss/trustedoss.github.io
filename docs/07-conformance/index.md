...
충족 체크리스트:
  ...
  - "ISO/IEC 18974: G4.2 (3.4.1), G4.3 (3.4.2), G4.4 (3.4.2)"
셀프스터디 소요시간: 2시간
워크숍 소요시간: 15분 (M6 모듈 후반부)
---
...
갭 분석에서 미충족 항목이 나왔다고 당황하지 않아도 된다. 각 항목에는 어느 챕터로 돌아가면 되는지 링크가 포함되어 있다. 부분충족 항목은 작은 수정만으로도 충족으로 전환할 수 있는 경우가 많다.
...
> 이 단계는 ISO/IEC 5230 G4.1 (3.6.1) 및 ISO/IEC 18974 G4.2 (3.4.1) 요구사항을 충족합니다.
...
:::tip 워크숍 모드 (M6 후반부 - 15분)
이 단계에 도달했다면 모든 output/ 산출물이 완료된 상태여야 합니다. 아직 이전 단계 산출물이 누락된 경우 해당 모듈로 돌아가 먼저 완성하십시오.
:::

**진행 순서:**

1. **(5분) 자체 인증 선언의 의미 설명**
   - 강사가 OpenChain 자체 인증의 의미와 업계 활용 사례를 소개한다.
   - 자체 인증 기업 목록(https://www.openchainproject.org/conformance)을 함께 확인한다.

2. **(10분) agent 실행 및 갭 분석 리뷰**
   - `cd agents/07-conformance-preparer && claude` 를 실행하여 갭 분석 리포트를 생성한다.
   - 생성된 `gap-analysis.md`와 `declaration-draft.md`를 그룹별로 검토한다.
   - 미충족 항목이 있다면 어느 챕터에서 보완할 수 있는지 함께 확인한다.

3. **OpenChain 등록은 강의 후 개별 진행**
   - 실제 OpenChain 웹사이트 등록은 수강자가 강의 후 개별적으로 진행한다.
   - `output/conformance/submission-guide.md`가 단계별 안내를 제공한다.
...
> 이 단계는 ISO/IEC 5230 G4.3 (3.6.2) 및 ISO/IEC 18974 G4.3 (3.4.2) 요구사항을 충족합니다.
