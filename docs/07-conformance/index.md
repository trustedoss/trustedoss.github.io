...
18974는 알려진 취약점이 하나도 없을 것을 요구하지 않습니다. 식별된 취약점마다 취한 조치(조치가 필요 없다고 판단한 경우 포함)가 기록되어 있고(§4.3.2.2), 프로그램이 요구사항을 충족한다는 문서가 있으면(§4.4.1.1) 됩니다. 취약점이 있는 경우 다음과 같이 처리합니다:
