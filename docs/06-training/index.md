...
두 표준 모두 담당자와 관련 구성원이 교육을 이수했다는 증빙을 요구합니다. 정책 문서와 프로세스 절차서를 아무리 잘 갖춰도, 구성원이 그 내용을 모른다면 실질적인 컴플라이언스는 이루어지지 않습니다.

## 2. 배경 지식: 왜 교육이 표준 요구사항인가

### 정책과 프로세스만으로는 부족한 이유

정책과 프로세스가 있어도 사람이 모르면 동작하지 않습니다. ISO/IEC 5230과 18974는 단순히 문서 존재 여부만 확인하지 않습니다. 실제로 해당 문서를 알고, 이해하고, 따를 수 있는 구성원이 있는지를 함께 점검합니다. 교육 이수 기록은 자체 인증의 핵심 증거 중 하나입니다.

### ISO/IEC 5230과 18974의 교육 요구사항

**ISO/IEC 5230 (라이선스 컴플라이언스)**

- **3.1.2 (G1.4)**: 프로그램 참여자가 담당 역할 수행에 필요한 역량을 갖추고, 교육 이수 같은 증빙을 유지해야 합니다.
- **3.1.3 (G2.3)**: 프로그램 참여자가 정책 목표, 기여 및 관련 역할을 이해하도록 해야 합니다.
- **3.1.3 (G1.7)**: G2.3이 "인식 프로그램을 실제로 운영하는 것"이라면, G1.7은 "누가 그
  교육을 이수했는지 기록으로 남기는 것"입니다. `output/training/completion-tracker.md`가
  이 기록 역할을 합니다.

**ISO/IEC 18974 (보안 보증)**

- **4.1.2 (G1.4)**: 프로그램 참여자가 보안 보증 역할 수행에 필요한 역량을 갖추고, 그 증빙을 유지해야 합니다.
- **4.1.3 (G2.3)**: 프로그램 참여자가 보안 보증 정책의 목표와 자신의 역할을 이해하도록 해야 합니다.
- **4.1.3 (G1.7)**: 위와 동일하게, 보안 보증 교육을 누가 이수했는지 기록을 남겨야 합니다.

> 이 단계는 ISO/IEC 5230 G1.4 (3.1.2), G1.7 (3.1.3), G2.3 (3.1.3) 및 ISO/IEC 18974 G1.4 (4.1.2), G1.7 (4.1.3), G2.3 (4.1.3) 요구사항을 충족합니다.
...
교육 이수 기록은 별도의 도구 없이 스프레드시트나 마크다운 파일로 간단하게 관리할 수 있습니다. 중요한 것은 형식이 아니라 기록 자체입니다.
...
1. 이 문서를 처음부터 끝까지 읽습니다.
2. 아래 명령어로 training-manager agent를 실행합니다:
...
4. agent의 3가지 질문에 답합니다 (위 대화 예시 참조).
...
이 챕터를 완료하면 `output/training/` 폴더에 아래 파일이 생성됩니다:
...
교육 체계 구축을 완료했다면, 이제 자체 인증 선언을 준비할 차례입니다.

마지막 챕터로 이동하거나 agent를 바로 실행합니다.
