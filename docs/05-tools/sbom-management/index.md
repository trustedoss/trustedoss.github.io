...
:::tip 워크숍 모드 (M4 - 45분)
납품처의 SBOM 요구사항(포맷, 제공 주기, 계약 조건)을 미리 파악해오면 더 실질적인 문서를 생성할 수 있습니다.
:::

**핵심 3단계:**

**1단계 (10분) — SBOM 관리의 중요성 이해**

강사가 아래 내용을 중심으로 설명한다:
- outdated SBOM의 위험 사례 (앞의 배경 지식 섹션 참고)
- EO 14028 (미국 연방정부 조달), EU CRA (2027년 시행) 규제 동향
- SBOM을 "살아있는 문서"로 유지하기 위한 CI/CD 자동화 전략

**2단계 (25분) — agent 실행 및 문서 생성**
...
각 참가자가 자신의 상황에 맞게 agent의 질문에 답변하고, 두 개의 산출물을 생성한다. 강사는 순회하며 납품처 요구사항이 불명확한 참가자를 지원한다.

**막혔을 때:** 납품처가 없거나 요구사항이 불명확하면 "외부 제공 없음"으로 답변한다. agent가 내부 관리 중심의 계획을 생성한다.

**3단계 (10분) — sbom-sharing-template.md 리뷰 및 시나리오 논의**

생성된 공유 템플릿을 화면에 띄우고, 실제 납품 시나리오를 논의한다:
- "이 고객사에서 SBOM을 요청하면 어떤 절차로 제공할 것인가?"
- "계약서에 SBOM 제공 조항이 없는 경우, 먼저 제안하는 게 좋은가?"
- "제공한 SBOM에서 취약점이 발견되면 어떻게 대응할 것인가?"
