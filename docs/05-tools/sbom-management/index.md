...
SBOM 을 제공하는 것만큼, 외부에서 **납품받는** 소프트웨어의 SBOM 을 요구하고 검증하는 것도
중요합니다. 협력사가 개발한 코드에도 오픈소스가 들어 있고, 그 위험은 우리 제품의 위험이 됩니다.

- **계약에 반영**: 납품물에 SBOM(CycloneDX 또는 SPDX) 동반 제출을 계약·발주 조건에 명시합니다.
- **받은 SBOM 검증**: 받은 SBOM 을 그대로 믿지 않고, SCA 도구로 라이선스·취약점을 재확인합니다.
  소스를 받을 수 없는 경우에도 전달받은 바이너리나 컨테이너를 SCA 로 스캔해 대조합니다.
- **불일치 대응**: SBOM 과 실제 구성이 다르거나 위험이 발견되면 협력사에 공유해 해소를 요구합니다.

받은 SBOM 을 취약점 관리에 연결하는 방법은 [5.3 취약점 분석과 대응](../vulnerability/index.md)을,
상시 운영 검증에는 [TRUSCA](https://github.com/trustedoss/trusca)(받은 SBOM 을 수입해 적합성을
