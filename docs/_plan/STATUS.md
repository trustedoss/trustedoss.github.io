...
- Phase 2: Lighthouse CI 워크플로우(대표 6페이지, warn 예산, 커밋 b975123),
  ...
  harness-guide §9에 기록
- 잔여(선택): contextualSearch 활성화는 Algolia crawler recordExtractor에 Docusaurus
  facet(docusaurus_tag, language)을 추가해야 하며 대시보드 편집기 수작업이 필요해 보류.
  현재는 ko와 en 검색 결과가 섞여 나온다.
