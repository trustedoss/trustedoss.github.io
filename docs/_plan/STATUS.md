...
- `00-overview/index.md` 본문 축약(#15 계획 항목) 보류 — 온보딩 진입은 quick-start + 랜딩 CTA + 사이드바 순서로 달성. index 축약은 별도 콘텐츠 작업으로.
- **en 로케일 i18n quirk(기존)**: en에서 chapter index.html이 빌드되지 않아, en 폴백 페이지의 상대 `.md` 링크가 broken으로 경고된다(현재 quick-start/start-path 2건). KO는 0 broken, build 성공(warn), verify 12/12. en 로케일 정합은 별도 과제로 조사.
- checklist-mapping 사이드바 라벨을 "표준 요구사항 한눈에"로 변경(#17 사이드바 정리 시).

## 다음 작업 (#16 AI 에이전트 허브)

1. 신설 페이지 `docs/00-overview/agents.md`(또는 reference): 에이전트↔챕터↔산출물 매핑표 + 상황별 추천 + 공통 실행 주의(`:::tip 실행 전 확인`).
2. 사이드바 "시작하기" 끝에 추가(라벨: AI 에이전트로 산출물 만들기).
3. 랜딩/개요에서 허브로 링크. build/verify/캡처.
...
- 정적 데모 링크는 `pathname:///tools/<file>.html` 형식(브로큰 링크 체커 회피).
...
- #15: 정적 데모는 `pathname://`로 링크해야 broken 회피. en 로케일은 index 미빌드 quirk 존재(ko 무관).
