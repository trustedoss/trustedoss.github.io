...
## 5.4 AI SBOM 실습 챕터 (2026-07-09 승인) — 완료 (실측 1건 사용자 대기)

> 설계: followup-plan.md "추가 승인 과제" 절. 메인 BomLens, 대안 OWASP AIBOM Generator(KWG 링크).

- [!] C1: BomLens 모델 스캔 실측 — 외부 저장소 스크립트 실행이 auto 모드에서 차단되어 **사용자 실행 대기**.
  실행 방법: sbom-tools 클론 후 챕터 2단계 명령 실행, 산출물 생성 확인. 명령은 공식 문서와 축자 대조 완료(게이트 판정 2번)
