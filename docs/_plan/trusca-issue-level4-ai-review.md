...
| 단계                  | 상태            | 근거                                                                                                                                                      |
| --------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3단계 CI/CD 자동 차단 | 다섯 영역 중 넷 | `secret-scan.yml`(gitleaks) · `sast.yml`(bandit·semgrep) · `codeql.yml` · `sca-self.yml`(cdxgen+Trivy) · `ci.yml`의 `image-scan`(Trivy). IaC 는 대상 없음 |
| 4단계 AI 방어 레이어  | **없음**        | AI 리뷰 워크플로우 부재                                                                                                                                   |
| 5단계 지속 모니터링   | 갖춤            | `dependabot.yml`(5개 생태계) · `sca-self.yml`(매일 07:00) · `dogfood-scan.yml` · `demo-health-canary.yml`(30분). DAST 없음                                |
...
아래 이슈 본문에 그 방법을 두 가지로 정리했고 첫 번째를 권장했다.
...
Level 4 is the gap. The guide site links this repository as the working
example for levels 3 and 5, and notes that public examples of level 4 are
rare — which is precisely why adding one here would be useful.

### What level 4 adds

Levels 3 tools match known patterns accurately. Code written by an AI can
produce shapes no rule covers, and those pass. Level 4 answers that with a
...
3. Get back per-finding verdicts — true positive or false positive, risk
   level, and an exploitation path when real
4. Post the result as a PR comment

**This must not gate the build.** LLM verdicts carry a high false positive
rate; the value is in triage, not enforcement. `secret-scan` and `sast`
should remain the only blocking checks.

A working implementation with the full workflow file, the parsing code, and a
worked example of what goes to the model and what comes back:
...
The secrets context is not available in a job-level `if`, so move the check
to `env` and gate each step:
...
```yaml
# Level 4 — findings-driven AI review. Disabled pending an
...
Either way the prerequisite is the same and worth tracking separately: the
API key needs to be provisioned and its cost owner agreed.
...
against a self-hosted model (Ollama and similar) — only the client call
changes. Worth noting since air-gapped deployment is a stated use case for
this project.
...
- 이슈에 라벨을 붙인다면 `enhancement` 와 보안 관련 라벨이 맞다
- 우리 쪽 근거 페이지: https://trustedoss.github.io/ai-coding/ai-security-review
  (워크플로우 전문, 파싱 코드, 실제 입출력 예시가 모두 들어 있다)
- 이 제안이 반영되면 가이드의 4단계에도 실전 사례 링크를 붙일 수 있다.
  현재는 `website/ai-coding/strategy.md` 4단계 절에 "공개 사례가 드물다"고 적혀 있다
```
